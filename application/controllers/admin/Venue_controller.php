...
class Venue_Controller extends CI_Controller {
	...
	*/
    public function save_add_venue()
    {
	    ...
	    if ($this->form_validation->run()==true) {
		        ...
		        $savedata['venue_map'] = $this->input->post('venue_map',TRUE);
		       $this->venue_model->save_venue($savedata);
			   
			   $user_id = $this->session->userdata('log_id');
		$action_title = 'Add venue';
		$action_description = 'Admin add venue';
	...
	*/
    public function save_edit_venue()
    {
        ...
        if ($this->form_validation->run()==TRUE) {
           ...
           $user_id = $this->session->userdata('log_id');
			$action_title = 'Edit Venue';
			$action_description = 'Admin edit venue';
			$add_date = date('Y-m-d h:i:s');
			$sql_int = "insert into user_action_log (user_id,action_title,action_description,add_date) values ('$user_id','$action_title','$action_description','$add_date')";
			$this->db->query($sql_int);
	...
	*/
    public function delet_venue($id)
    {
	  ...
	  $user_id = $this->session->userdata('log_id');
		$action_title = 'Delete Venue';
		$action_description = 'Admin delete venue';
		$add_date = date('Y-m-d h:i:s');
		$sql_int = "insert into user_action_log (user_id,action_title,action_description,add_date) values ('$user_id','$action_title','$action_description','$add_date')";
		$this->db->query($sql_int);
