...
class Theraphists extends CI_Controller {
...
#------------------------------------------------
	...
	{
        $data['title'] = "Add New Theraphists";
		$this->load->view('admin/_header',$data);
		$this->load->view('admin/_left_sideber');
		$this->load->view('admin/websetting/add_theraphists');
...
#  post list
#------------------------------------------------- 
	public function theraphists_list($search=NULL)
	{
        $data['title'] = "Theraphists List";
        $data['post_info'] = $this->db->get('theraphists')->result();
       	$this->load->view('admin/_header',$data);
        $this->load->view('admin/_left_sideber');
        $this->load->view('admin/websetting/theraphists_list');
        $this->load->view('admin/_footer');
	}
...
#------------------------------------------------- 
  public function edit_post($id=NULL)
  {
        ...
        $data['post_info'] = $this->db->where('id',$id)->get('theraphists')->row();
        $this->load->view('admin/_header',$data);
        $this->load->view('admin/_left_sideber');
        $this->load->view('admin/websetting/edit_theraphists');
        $this->load->view('admin/_footer');
  }
...
public function save_edit_post()
{
    ...
    if ($this->form_validation->run()==true) {
        ...
        if (@$_FILES['picture']['name']){
            ...
            if (!$this->upload->do_upload('picture')){
                $this->session->set_flashdata('exception',"<div class='alert alert-danger msg'>".$this->upload->display_errors()."</div>");
                redirect('admin/theraphists/post_list');
            } else {
                    $data = $this->upload->data();
                    $image = base_url($config['upload_path'].$data['file_name']);
            ...
            $this->db->where('id',$id)->update('theraphists',$savedata);
            $this->session->set_flashdata('message','<div class="alert alert-success">'.display('update_msg').'</div>');
            redirect('admin/theraphists/theraphists_list');
        } else {
    ...
    {
        $this->db->where('id',$id)->delete('theraphists');
