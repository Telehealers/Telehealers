...
*/ 
	public function __construct() 
	{
	    ...
	    $this->load->model('admin/Resource_model','resource_model');
	}
...
#------------------------------------------------
	...
	{
    $data['title'] = "Add New Resource";
		$this->load->view('admin/_header',$data);
		$this->load->view('admin/_left_sideber');
		$this->load->view('admin/websetting/view_create_new_resource');
...
#------------------------------------------------- 
	public function resource_list($search=NULL)
	{
        ...
        $data['post_info'] = $this->resource_model->get_all_post();
       	$this->load->view('admin/_header',$data);
        $this->load->view('admin/_left_sideber');
        $this->load->view('admin/websetting/view_resource_list');
        $this->load->view('admin/_footer');
	}
          ...
          if (@$_FILES['picture']['name']){
              ...
              if (!$this->upload->do_upload('picture')){
                  $this->session->set_flashdata('exception',"<div class='alert alert-danger msg'>".$this->upload->display_errors()."</div>");
                    redirect('admin/Resource_controller');
              } else {
			...
			$action_title = 'Add Resource';		
			
			$action_description = 'Admin add new Resource';
...
#------------------------------------------------- 
  public function edit_post($id=NULL)
  {
    ...
    $data['post_info'] = $this->resource_model->get_post_by_id($id);
    $this->load->view('admin/_header',$data);
    $this->load->view('admin/_left_sideber');
    $this->load->view('admin/websetting/view_edit_resource');
    $this->load->view('admin/_footer');
  }
...
public function save_edit_post()
{
    ...
    if ($this->form_validation->run()==true) {
        ...
        if (@$_FILES['picture']['name']){
            ...
            if (!$this->upload->do_upload('picture')){
                $this->session->set_flashdata('exception',"<div class='alert alert-danger msg'>".$this->upload->display_errors()."</div>");
                redirect('admin/resource_controller/resource_list');
            } else {
                    $data = $this->upload->data();
                    $image = base_url($config['upload_path'].$data['file_name']);
			...
			$action_title = 'Update resource';		
			
			$action_description = 'Admin update resource';	
...
#------------------------------------------------- 
    ...
    {
        $this->db->where('id',$id)->delete('resource_tbl');
