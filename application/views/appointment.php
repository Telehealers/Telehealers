...
<body>
  <?php $this->load->view('header.php')?>
    ...
    <?php   $attributes = array('class' => 'form-horizontal','role'=>'form');
      ...
      ?>
    <div id="q_succ_msg" class="mt-3 alert alert-warning" style="display:none"></div>
...
$('#q_succ_msg').html('No Consultants available at this time. Watch tutorial video below.');
$('#q_succ_msg').show();
if (IsMobile()){
    $('#tutorialPhone').show();
} 
...
function getDoctors(language,date,hour,min,am_pm){
    ...
    success: function(response){
      if(!$.trim(response)){
        $('#q_succ_msg').html('No Consultants available at this time');
        $('#q_succ_msg').show();
        showTutorial();
        }
      else{
